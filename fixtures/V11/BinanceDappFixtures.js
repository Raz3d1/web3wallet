// fixtures/V11/BinanceDappFixtures.js — Binance 特供版：BSC 默认占位，可改 buildFixtureFromData 的 vars
(function () {
  ...
  /** BSC 默认；占位符见 fixtures-data/V11.js */
  function bscVars(address) {
  ...
  function v11_RequestAccountsFixture(addr) {
    return g.buildFixtureFromData("Dapp_RequestAccounts", bscVars(addr));
  }

  function v11_RequestAccountsFullFixture(addr) {
    return g.buildFixtureFromData("Dapp_RequestAccounts_Full", bscVars(addr));
  }

  function v11_SendTransactionFixture(addr) {
    return g.buildFixtureFromData("Dapp_SendTransaction", bscVars(addr));
  }

  function v11_SignTransactionFixture(addr) {
    return g.buildFixtureFromData("Dapp_SignTransaction", bscVars(addr));
  }

  function v11_SignPersonalMessageFixture(addr) {
    return g.buildFixtureFromData("Dapp_SignPersonalMessage", bscVars(addr));
  }

  function v11_SignMessageFixture(addr) {
    return g.buildFixtureFromData("Dapp_SignMessage", bscVars(addr));
  }

  function v11_EthSignTypedDataV4Fixture(addr) {
    return g.buildFixtureFromData("Dapp_EthSignTypedData_V4", bscVars(addr));
  }

  function v11_SignRawTransactionFixture(addr) {
    return g.buildFixtureFromData("Dapp_SignRawTransaction", bscVars(addr));
  }

  function v11_EcRecoverFixture(addr) {
    return g.buildFixtureFromData("Dapp_EcRecover", bscVars(addr));
  }

  function v11_AddEthereumChainFixture(addr) {
    return g.buildFixtureFromData("Dapp_AddEthereumChain", bscVars(addr));
  }

  function v11_CommonJsonRpcEthChainIdFixture(addr) {
    return g.buildFixtureFromData("Dapp_CommonJsonRpc_eth_chainId", bscVars(addr));
  }
